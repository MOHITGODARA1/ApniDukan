...
const UserRegister = async (req, res) => {
  try {
    ...
    return res.status(201).json(
      new Apiresponce(201, userCreated, "User created successfully")
    );
...
const UserLogin=async(req,res)=>{
  try {
    ...
    //send OTP
    //acess token refreshtoken
    const {acesstoken,refreshToken}=await genrateacessTokenandrefreshtoken(user._id);

    const userlogedin=await Register.findById(user._id).select(
      "-refreshToken"
    )
    //cookie
    const option={
    ...
    .json(
       new Apiresponse(200,"login sucessful")
    )
